...
export class GraylogClient {
  ...
  private authHeader(): string {
    if (this.connection.authType === 'basic') {
      if (!this.connection.username || !this.connection.password) {
        throw new Error(`Log connection "${this.connection.id}" is authType=basic but missing username/password`);
      }
      return `Basic ${Buffer.from(`${this.connection.username}:${this.connection.password}`).toString('base64')}`;
    }
    if (!this.connection.token) {
      throw new Error(`Log connection "${this.connection.id}" is authType=bearer but missing token`);
    }
   ...
   * fixed, possibly days-old incident window work at all.
   */
