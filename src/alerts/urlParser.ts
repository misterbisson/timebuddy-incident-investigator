export interface ParsedDashboardUrl {
  ...
  to?: string;
  /**
   * The link's own `timezone` param, when set. Only meaningful alongside a
   * relative `from`/`to` that rounds to a period ("now/d", "now/w-7d"): those
   * name a wall-clock boundary, so the zone the link was authored in decides
   * which instant they mean. See query/dateMath.ts.
   */
  timezone?: string;
}
