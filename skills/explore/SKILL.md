...
1. Call `find_related_dashboards` with no arguments (no `metricName`, no `labels`, no
   ...
   configured connection's metric index — use `dashboardsScanned` and `brokenDatasources` per
   connection from the response as your real connectivity signal, not the (empty) `matches` list.
...
3. If it succeeds, summarize in plain language, not raw JSON: how many connections responded,
   roughly how many dashboards each one has, and call out plainly if any connection reported
   broken datasource references (a real, fixable Grafana-side issue worth surfacing).
